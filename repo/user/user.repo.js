...
exports.getAddresses = (user) => {
  return user.getAddresses({ order: [[sequelize.literal('field(is_default, 1)'), 'desc'], ['created_at', 'desc']] })
    .then(address => {
      return {
        'addresses': address,
        'default_values': {
...
exports.defaultAddress = (id, userId) => {
  ...
  return Promise.all([address, allUpdatedAddress])
    .then(([address, allUpdatedAddress]) => {
      return new Promise((resolve, reject) => {
...
exports.active = (user_id) => {
  return DeliveryBoy.findByPk(user_id).then(user => {
    user.isActive = true
    user.save();
    return user;
  }).catch(err => {
