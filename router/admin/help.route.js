...
router.delete("/link/pending/rh/:id", (req, res) => {
  helpRepo
    .getRhLinkById(req.params.id)
    .then(async (rhLink) => {
      if (rhLink.status !== "pending") {
        throw new ResMessageError("rh is not pending");
      }
      await rhLink.destroy();
      res.json(new CommonResponse((code = 200), (message = "Pending RH link deleted"), (data = rhLink)));
    })
    .catch((err) => {
      res.status(400).json(new CommonResponse((code = 400), (message = err.message)));
    });
