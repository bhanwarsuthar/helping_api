...
router.post('/update/profile', Auth, validator(schema.update_user), (req, res) => {
    ...
    userRepo.update_user(req.user, req.body)
        .then(user => {
...
router.get('/addresses', Auth, (req, res) => {
    userRepo.getAddresses(req.user)
        .then(address => {
...
router.post('/addresses', Auth, validator(schema.new_address), (req, res) => {
    userRepo.addAddress(req.user.id, req.body,)
        .then(address => {
...
router.put('/addresses/:id/default', Auth, (req, res) => {
    userRepo.defaultAddress(req.params.id, req.user.id)
        .then(address => {
...
router.post('/addresses/:id/update', Auth, (req, res) => {
    userRepo.updateAddress(req.params.id, req.user.id, req.body)
        .then(address => {
            res.json({ message: "", data: address })
        })
        .catch(err => {
...
router.delete('/addresses/:id', Auth, (req, res) => {
    userRepo.deleteAddress(req.params.id, req.user.id)
        .then(() => {
...
router.post('/check_sponsor', validator(schema.sponsor_code), (req, res) => {
    userRepo.checkSponsor(req.body.sponsor_code)
        .then(user => {
            res.json({ message: "", data: user })
        })
        .catch(err => {
...
router.post('/attach_sponsor', Auth, validator(schema.sponsor_code), (req, res) => {
    userRepo.attachSponsor(req.user.id, req.body.sponsor_code)
        .then(user => {
            res.json({ message: "", data: user })
        })
        .catch(err => {
...
router.get('/wallets/:slug/history', Auth, 
    async (req, res) => {
        var user = req.user;
